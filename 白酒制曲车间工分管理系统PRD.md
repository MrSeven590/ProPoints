...
- **数据存储（一期实现）**：使用 `uni.*Storage` 的 **key-value 本地持久化**作为主存储（App 端为持久化存储；小程序端通常存在容量限制，例如微信小程序常见为“单 key 1MB、总量 10MB”（以平台策略为准），需按“录入单/日期/环节”拆分存储）。
...
- **序号上限（seq_max）规则**：
  ...
  - 若无任何历史数据，则取“班级配置”中该楼层的**默认仓数**作为 `seq_max`（支持分别配置 **3 楼 / 4 楼 / 5 楼**）；若未配置则 **默认 `24`**。
- **智能推断（录入页默认）**：
...
#### 2.1.3 发酵仓楼层默认仓数配置（3/4/5 楼）

- 由于现场发酵仓主要集中在 **3 楼 / 4 楼 / 5 楼**，班级配置页需支持分别设定：
  - `3楼默认仓数`
  - `4楼默认仓数`
  - `5楼默认仓数`
- 用途（关键口径）：
  - 当某楼层**没有任何历史数据**时，该楼层的 `seq_max` 取对应楼层的“默认仓数”（用于智能推断跨楼层、仓选择器网格上限等）。
  - 当某楼层已有历史数据时，仍以“历史最大 `seq`”作为 `seq_max`（默认仓数仅作为无历史的兜底）。
- 默认值：未配置时默认 `24`。
- **存储口径（一期）**：保存到 `uni.*Storage` 的 `pp:cfg:binSeqMaxByFloor`（示例：`{"3":24,"4":24,"5":24}`）。
...
### 8.5 本地存储与云同步（路线建议，适配 App + 小程序）

#### 8.5.1 一期：Local-First（离线主用）

- **本地主库**：以 `uni.*Storage` 作为主存储（key-value），将“一次录入单/会话”序列化为 JSON 存储。
- **Key 设计（建议）**：按“日期/环节/轮次”拆分，避免单个 key 过大，便于小程序端兼容。
  - 小程序端需控制单条 key 的数据体积（微信小程序常见为单条 1MB），也需控制总量（常见为 10MB，具体以平台策略为准）。
  - `pp:session:{YYYY-MM-DD}:{stage}:{roundId}`：会话主数据（含 bins/assignments/penalties 等）
  - `pp:idx:date:{YYYY-MM-DD}`：当日会话 key 列表（用于快速查询/公示/导出）
  - `pp:cfg:*`：班级配置、系数版本、上次堆曲仓号、安曲晾堂默认人员等
    - `pp:cfg:binSeqMaxByFloor`：3/4/5 楼默认仓数配置（示例：`{"3":24,"4":24,"5":24}`）
- **离线备份（建议增强）**：提供“导出 JSON / 导入 JSON”作为人工备份与跨设备迁移的兜底方案。

#### 8.5.2 二期：适时上云（多人/多设备同步）

- **触发条件**：需要多设备共享数据、账号登录、换手机不丢数据、多人协作时再上云；弱网现场仍以本地录入为主。
- **同步策略建议**：
  - 继续保留本地为主库；云端作为“同步副本”；
  - 使用预留字段 `uuid/version/sync_status/updated_at/deleted_at/device_id` 做增量同步与冲突检测；
  - 维护 `pp:oplog`（本地变更队列）：记录新增/修改/删除操作，联网后批量 push；云端返回最新变更用于 pull。
- **云端选型建议（更适合初学者）**：
  - 优先选择“带鉴权 + 关系型查询”的方案作为云端主库（便于按日期/人员/仓查询与导出），例如：Supabase（Postgres+Auth）或国内云开发/Serverless DB。
  - Cloudflare D1（云端 SQLite）作为备选：适合 SQL 查询，但仍需自行实现 API、鉴权与同步。
  - 不建议将 Serverless KV 作为主库（更适合配置/缓存/索引），否则复杂查询与统计成本会转嫁到客户端索引维护。
