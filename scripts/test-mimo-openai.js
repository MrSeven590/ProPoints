/**
 * MiMo API 连通性测试脚本（OpenAI Chat Completions 协议）
 *
 * 复现 MiMoOcrService.uts 中 buildOcrInstructions 的提示词，改用 OpenAI Chat Completions API 协议：
 *   - 端点: /v1/chat/completions
 *   - 系统提示词放 { role:"system", content:"..." }
 *   - 图片用 { type:"image_url", image_url:{ url:"data:image/png;base64,..." } } 格式
 *   - JSON 约束用 response_format:{ type:"json_object" }
 *   - 响应从 choices[0].message.content 提取
 *
 * 测试三阶段：
 *   1. Key + Endpoint 可达（最小文本请求）
 *   2. OCR 提示词 + JSON 输出（纯文本模拟草稿内容）
 *   3. 图片输入管道（使用项目内 logo.png 作为占位图）
 *
 * 用法: node scripts/test-mimo-openai.js
...
const ENDPOINT = "https://fufu.iqach.top/v1/chat/completions";
...
// 通用请求函数
async function callOpenAI(body, timeoutMs = 90000) {
...
async function test1Connectivity() {
  divider("测试 1: 最小连通性验证（OpenAI Chat Completions 协议，纯文本对话）");
  const body = {
    model: MODEL,
    messages: [
      {
        role: "system",
        content: "You are MiMo, an AI assistant. 请用中文回复。",
      },
      {
        role: "user",
        content: '请回复"你好，我是 MiMo"，不要输出其他内容。',
      },
    ],
  ...
  const t0 = Date.now();
  const res = await callOpenAI(body, 60000);
  ...
  console.log("✅ 连通成功");
  const out = extractChatContent(res.data);
...
async function test2OcrJson() {
  ...
  console.log("system 长度      :", body.messages[0].content.length, "字符");
  console.log("response_format  :", JSON.stringify(body.response_format));
...
async function test3ImageInput() {
  divider("测试 3: 图片输入管道验证（OpenAI image_url + logo.png）");
  ...
  console.log(
    "image_url.url (前40字符)    :",
    body.messages[1].content[0].image_url.url.slice(0, 40) + "..."
  );
  console.log("请稍候（最长 90s）...");
  const t0 = Date.now();
  const res = await callOpenAI(body, 90000);
  console.log("耗时:", ((Date.now() - t0) / 1000).toFixed(1) + "s");
  console.log("HTTP 状态:", res.status);
  ...
  try {
    ...
    console.log("  uncertain_items :", JSON.stringify(parsed.uncertain_items));
    console.log("✅ 图片输入管道可用（API 接受 image_url 且返回 JSON）");
    return true;
  } catch (e) {
    ...
    if (out.length > 0) {
      ...
      console.log("✅ 图片输入管道可用（API 接受 image_url）");
      return true;
    }
    console.log("❌ 无有效文本输出");
    return false;
...
// ====== 主流程 ======
async function main() {
  console.log("MiMo API 连通性测试（OpenAI Chat Completions 协议）");
  ...
  console.log(
    allPass
      ? "\n🎉 全部测试通过，OpenAI Chat Completions 协议与中转站对接成功"
      : "\n⚠️  部分测试未通过，请查看上方日志"
  );
