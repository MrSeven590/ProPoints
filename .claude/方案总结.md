...
- 第一阶段：单机离线（`uni.*Storage` 本地持久化 key-value，Local-First；App 端持久化，小程序端需注意容量限制并按“录入单/日期/环节”拆分存储）
- 第二阶段：预留 `uuid/version/sync_status/updated_at` 等字段，引入“本地为主 + 云端增量同步”（弱网场景仍以本地录入为主）
...
- 到达楼层末尾时自动跳转到下一楼层
- **楼层末尾推断规则**：优先取该班级该楼层历史最大 `seq_no`；若无历史则取“班级配置”的该楼层默认仓数（支持分别配置 3/4/5 楼）；未配置再默认 `24`
...
说明：本章以“关系模型”描述业务实体与约束，便于理解与后续上云落库；一期在 uni-app-x 上的实际落地采用 `uni.*Storage` 做文档化存储（JSON + 索引 key），不依赖直接 SQLite SQL 调用。
...
- created_by_manager（管理员姓名快照）：创建本会话的管理员
- updated_by_manager（管理员姓名快照）：最后编辑本会话的管理员
- created_at, updated_at, version, sync_status（为二期同步预留）
...
- created_by_manager（管理员姓名快照）：创建本分配行的管理员
- updated_by_manager（管理员姓名快照）：最后编辑本分配行的管理员
- created_at, updated_at, version, sync_status（为二期同步预留）
...
CREATE TABLE ferment_round (
  ...
  year          INTEGER NOT NULL,  -- 年份（如 26 表示 2026 年）
  ...
  UNIQUE (class_no, year, round_no)
);
```

说明：
- `year` + `round_no` 组合标识轮次（如 26年第3轮）
- `round_no` 范围 1-7，每年最多 7 轮
- `started_at` 为轮次业务开始时间，`created_at` 为记录创建时间
...
### 3.3 存储方案
- **技术选型（一期）**：`uni.*Storage`（key-value 本地持久化，Local-First）。
  - App 端为持久化存储；小程序端通常存在容量限制（例如微信小程序常见为单 key 1MB、总量 10MB，以平台策略为准），因此必须按“录入单/日期/环节”拆 key。
- **数据组织（建议）**：
  - `pp:session:{YYYY-MM-DD}:{stage}:{roundId}`：会话主数据（包含 bins/assignments/penalties 等）
  - `pp:idx:date:{YYYY-MM-DD}`：当日会话 key 列表（用于按日期查询/公示/导出）
  - `pp:cfg:*`：班级配置、系数版本、上次堆曲仓号、安曲晾堂默认人员等
    - `pp:cfg:binSeqMaxByFloor`：3/4/5 楼默认仓数配置（示例：`{"3":24,"4":24,"5":24}`）
- **变更队列（为二期同步预留）**：维护 `pp:oplog`（操作日志队列），每次新增/修改/删除追加一条记录；联网时批量 push 到云端，云端返回增量用于 pull。
- **状态字段**：`draft/submitted`；公示为人工触发且不产生“锁定”，可选记录 `published_at` 作为“最近一次公示时间”。
- **本地优先**：第一阶段单机离线；核心数据统一预留 `uuid/version/sync_status/updated_at/deleted_at/device_id` 便于二期云端同步与冲突检测
- `is_synced` 可由 `sync_status` 推导（例如：`sync_status='synced'`）
  - 同步策略建议：本地为主库、云端为同步副本；弱网现场只要求“可录入/可查询/可公示”，联网后再做增量同步。
  - 云端选型建议：优先“带鉴权 + 关系型查询”的方案（如 Supabase/国内云开发/Serverless DB）；Cloudflare D1 作为备选；不建议用 Serverless KV 作为主库（更适合配置/缓存/索引）。
...
3. 计算楼层最大序号 `seq_max`：优先取该班级该楼层历史最大 `seq_no`；若无历史则取班级配置的楼层默认仓数（3/4/5 楼可分别配置）；未配置再默认 `24`
...
/pages
  ...
  /mine                  # 我的/设置
    ...
    class-config.uvue    # 班级配置：发酵仓设置（3/4/5楼默认仓数）/编号规则/轮次历史（轮次切换入口在安曲录入页）
...
- 跨楼层：当 `seq_no` 超过楼层最大值时自动跳转到下一楼层：
  - `seq_max` 取“该班级该楼层历史最大 seq”，若无历史则取班级配置的楼层默认仓数（3/4/5 楼可分别配置；未配置默认 `24`）；
  - 示例：`3-3-24` → `3-4-1`。
...
发酵仓网格（多选；网格上限展示至该楼层的 seq_max，可随历史扩展；若该楼层无历史数据则取班级配置默认仓数；每格展示完整编号）
```
┌─────────────────────────────────┐
...
/ProPoints
├── /storage
│   ├── storage-keys.uts        # Storage Key 约定
│   ├── storage-repository.uts  # `uni.*Storage` 读写封装（文档化存储）
│   └── oplog.uts               # 变更队列（为二期云同步预留）
├── /domain
│   ├── /models             # 实体类型定义
│   ├── /services
...
| 小程序端存储容量限制 | 按“录入单/日期/环节”拆 key；避免单 key 过大；提供 JSON 导出/导入作为备份兜底 |
...
| 决策项 | 推荐方案 | 备选方案 |
|--------|----------|----------|
| 数据存储 | `uni.*Storage`（Local-First） | 云端关系型DB（Supabase/云开发/D1）+ 本地增量同步 |
...
**字段设计**：
- `created_by_manager`: 创建者（管理员姓名快照）
- `updated_by_manager`: 最后编辑者（管理员姓名快照）
...
1. **创建会话/分配时**：
   ```typescript
   import { getManagerName } from '@/domain/stores/AppStore.uts'

   const managerName = getManagerName()
   const session: StageSessionCreateParams = {
     // ... 其他字段
...
2. **更新会话/分配时**：
   ```typescript
   const managerName = getManagerName()
   const updateParams: StageSessionUpdateParams = {
     // ... 其他字段
...
3. **读取旧数据兼容**：
   ```typescript
   // 旧数据可能没有这些字段，读取时视为空字符串
   const createdBy = session.created_by_manager ?? ''
   const updatedBy = session.updated_by_manager ?? ''
   ```

4. **查询支持**：
   - 某日某环节谁录入 → `StageSession.created_by_manager`
   - 某人某日工分谁录入 → `Assignment.created_by_manager`
   - 最后谁编辑过 → `*.updated_by_manager`

**注意**：
- 存储的是**姓名快照**，不是引用。即使管理员姓名后续变更，历史记录仍保留当时的姓名。
- 管理员姓名通过 `AppStore.getManagerName()` 获取，来源于 `pp:cfg:manager` 存储。
- 若管理员姓名未设置，字段值为空字符串（不影响录入流程，但建议在设置页提示用户填写）。
