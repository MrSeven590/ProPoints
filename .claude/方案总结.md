...
/pages
  ...
  /stats                 # 数据查询与报表
    index.uvue           # 统计概览
    date-detail.uvue     # 按日期：每日公示/简报 + Excel导出入口
...
#### 5.5.1 每日公示（按日期）
- `date-detail.uvue`：展示当日所有人员的最终工分（可切换显示 原始/扣分/最终）。
- 页面右上角提供【导出Excel】按钮（手动触发）。
