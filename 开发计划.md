...
- 涉及的文件/模块：
  - `domain/services/AnQuDataService.uts`（`lookupKojiCount` / `lookupKojiCountBatch` 相关）
...
- 语义约定：一期 `AssignmentSource` 是按仓曲坯占比和跨仓工分自动派生的算法快照，用于保存/回显/校验一致性，不提供用户手工编辑来源分摊；若后续支持人工调整，需拆分“建议来源”和“确认来源”，避免自动重算覆盖用户确认值。
- 涉及的文件/模块：
